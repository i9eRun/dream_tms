...
Ext.define('dream.view.tbas.tbas1008ViewController', {
    ...
    onTbas1008_dept_search_keywordAfterRender: function(component, eOpts) {
    ...
    onTbas1008searchClick1: function(button, e, eOpts) {

        const grid = this.lookupReference('tbas1008_dept_grid');
        const store = grid.getStore();
        const use_yn = this.lookupReference('use_yn').getValue();
    ...
    onGridpanelSelectionChange_tbas1008_dept_grid: function(model, selected, eOpts) {
        ...
        deptCodeField.setReadOnly(true);
    },
    ...
    onFormAfterRender_tbas1008_dept_form: function(component, eOpts) {
        const fields = [
        "N_DEPT_CD",
        "DEPT_NM",
        "JIYUK_CD",
    ...
    onInsert_deptClick_tbas1008: function(button, e, eOpts) {
        ...
        const deptCodeField = form.down('[name=N_DEPT_CD]');
        if (deptCodeField) {
            deptCodeField.setReadOnly(false);
    ...
    onTbas1008_dept_saveClick: function(button, e, eOpts) {
        ...
        const nDeptCdField = form.down('[name=N_DEPT_CD]');
        const deptNmField  = form.down('[name=DEPT_NM]');
        const useYnField   = form.down('[name=USE_YN]');

        if (Ext.isEmpty(nDeptCdField.getValue())) {
            Ext.Msg.alert('알림', '부서코드를 입력하세요.', function() {
                nDeptCdField.focus(true, 200);
            });
            return;
        }

        if (Ext.isEmpty(deptNmField.getValue())) {
            Ext.Msg.alert('알림', '부서명을 입력하세요.', function() {
                deptNmField.focus(true, 200);
            });
            return;
        }

        if (Ext.isEmpty(useYnField.getValue())) {
            Ext.Msg.alert('알림', '사용여부를 선택하세요.', function() {
                useYnField.focus(true, 200);
            });
            return;
        }
        ...
        Ext.Msg.confirm('확인', '부서 정보를 저장하시겠습니까?', function(btn) {
            if (btn === 'yes') {
                Ext.Ajax.request({
